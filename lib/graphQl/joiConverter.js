...
joiConverter.simpleAttribute = (joiScheme, resourceName, attributeName) => {
  ...
  if (type === 'array' || type === 'alternatives') {
    ...
    const types = items.map( item => joiConverter.simpleAttribute(item, resourceName, `${attributeName}_${item._type}`) )
    ...
    if (types.length === 0) {
      ...
      const union = new graphQl.GraphQLUnionType({
        name:`${resourceName}_${attributeName}`,
        types,
        resolveType(value) {
...
joiConverter.shallowInput = (joiScheme, allWriteTypes, resourceName, attributeName) => {
  let type
  if (!joiScheme._settings) {
    type = joiConverter.simpleAttribute(joiScheme, resourceName, `shallow_${attributeName}`)
  } else {
